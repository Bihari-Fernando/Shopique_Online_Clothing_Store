...
const ItemView = () => {
  ...
  const getFullUrl = (url) => (url?.startsWith('http') ? url : `${BASE_URL}${url}`);

  // Determine the main image to show (default to product.image)
  const displayedMainImage = mainImage ? getFullUrl(mainImage) : getFullUrl(product.image);
      ...
      <div className="flex flex-col md:flex-row items-start gap-10">
              ...
              {[product.image, ...galleryImages].map((img, idx) => (
                <img
                  ...
                  className={`w-20 h-20 object-cover rounded cursor-pointer border-2 ${
                    getFullUrl(img) === displayedMainImage ? 'border-blue-500' : 'border-transparent'
                  }`}
        ...
        <div className="flex flex-col gap-4 flex-1">
          ...
          <div className="flex gap-3 mt-4">
            <Button onClick={ handleAddToCart } className="bg-green-500 hover:bg-gray-600 text-white font-semibold px-4 py-2 rounded shadow">
              Add to Cart
            </Button>
            <Button onClick={() => navigate(-1)}  className="bg-gray-500 hover:bg-gray-600 text-white font-semibold px-4 py-2 rounded shadow">
              Back to Products
            </Button>
          </div>
        </div>
